...
**Why it matters.** If we are a CRA, the FCRA compliance burden inverts onto us (§ 1681e accuracy/maximum-possible-accuracy procedures, dispute/reinvestigation under § 1681i, user certifications, etc.). _(Correction per counsel: a CRA and a furnisher are distinct statutory roles — Regulation V excludes an entity from the furnisher definition while it is acting as a CRA. CRA status does **not** automatically impose furnisher duties unless we separately furnish consumer information to another CRA. The original draft of this brief wrongly lumped "furnisher duties" in here; that was incorrect and is struck.)_ This is potentially product-shape-ending for a small D2C launch.
...
**CCPA/CPRA** applies to "businesses" meeting thresholds. _(Correction per counsel: the original draft overstated the Sensitive Personal Information reach. A credit report is unquestionably personal information, but **not every field is automatically SPI** — SSNs and government identifiers qualify; financial account information generally qualifies as SPI only **when combined with credentials, security codes, or access information**. Treat "SPI = whole report" as wrong.)_ SSNs/credentials/unnecessary identifiers should be redacted before analysis. Data minimization requires collection/use/retention reasonably necessary and proportionate to the disclosed purpose — driving retention classes, deletion design, and the "do we even need full account numbers/SSNs" question. Note also counsel's exemption trap: California's FCRA/GLBA exemptions are narrow and activity-specific — we cannot claim "not a CRA" *and* rely on the FCRA exemption simultaneously, and the private breach remedy sits outside both.
