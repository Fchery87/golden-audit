...
Automated criteria are covered by `tests/pilot-readiness.test.ts`, `tests/platform.test.ts`, and `npm run verify:pilot`. Vendor review and accountable product/legal/privacy/security/operations/accessibility approvals are external human gates and remain open in `docs/pilot-readiness.md`; real consumer reports must not be used until they are signed.
